...
const DeleteConfirmationModal = ({ isOpen, toggle, onDelete, title, message, itemName, itemType }) => {
  const displayMessage = message || (itemName && itemType ? 
    `Tem certeza que deseja excluir ${itemType === 'o' ? 'o' : 'a'} ${itemType} "${itemName}"?` : 
  ...
  return (
    <Modal isOpen={isOpen} toggle={toggle}>
      ...
      <ModalBody>
        ...
        <div className="w-100">
          <Button color="danger" className="me-2" onClick={onDelete}>
            Excluir
          </Button>
