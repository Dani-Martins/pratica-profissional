...
const MarcaForm = ({ onSaved, onCancel, isModal }) => {
  ...
  useEffect(() => {
    if (isEdit) {
      setLoading(true);
      axios.get(`/api/marca/${id}`)
        .then(res => {
          setNome(res.data.marcaNome || res.data.nome || '');
          setAtivo(!!res.data.situacao);
        })
        .catch(() => setError('Erro ao carregar marca.'))
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await axios.put(`/api/marca/${id}`, marcaPayload);
        setSuccess('Marca atualizada com sucesso!');
      } else {
        const res = await axios.post('/api/marca', marcaPayload);
      ...
      setTimeout(() => navigate('/produtos/marca'), 1200);
    } catch (err) {
    ...
    <Card>
      <CardBody>
        <div className="d-flex justify-content-between align-items-center mb-4">
          <h4 className="mb-0">{isEdit ? 'Editar Marca' : 'Nova Marca'}</h4>
        </div>
        ...
        <Form onSubmit={handleSubmit}>
          <FormGroup>
            <Label for="marca">Marca <span className="text-danger">*</span></Label>
            <Input
              type="text"
              id="marca"
              value={nome || ''}
          ...
          <div className="d-flex justify-content-end mt-4">
            ...
            <Button color="secondary" onClick={() => navigate('/produtos/marca')} disabled={loading}>
              Cancelar
            </Button>
          </div>
        </Form>
